...
func TestAuditMiddleware_UnauthenticatedMutationIsLogged(t *testing.T) {
	...
	rec := doAPIFrom(cs, http.MethodDelete, "/api/rules/gfwlist", "198.51.100.9:1", "", false)
	...
	for _, want := range []string{"method=DELETE", "path=/api/rules/gfwlist", "src=198.51.100.9", "status=401"} {
...
// category/entry that 404s should be logged as status=404.
func TestAuditMiddleware_CapturesRealStatus(t *testing.T) {
	cs, token := newAPITestServer(t)
	buf := captureAuditLog(t)

	rec := doAPIFrom(cs, http.MethodDelete, "/api/subscriptions/does-not-exist", "203.0.113.5:1", token, true)
	if rec.Code != http.StatusNotFound {
		t.Fatalf("DELETE unknown subscription status = %d, want 404; body=%s", rec.Code, rec.Body.String())
	}
...
func TestAuditMiddleware_DoesNotLogRequestBody(t *testing.T) {
	...
	body := []byte(`{"id":"secret-sub","name":"n","category":"gfwlist","url":"https://example.com/super-secret-list","format":"plain","interval":"1h"}`)
