...
基于 **smartdns** 的 DoT 网关:**解析即策略**——靠 DNS 解析结果直接决定一条流量是直连还是进代理,被代理的流量经 xray 透明转发后**直出**。客户端仅通过 DoT(853)接入,无需安装任何 App。

> 仅用于已获合法授权的企业组网与技术研究,请遵守所在地法律法规。

## 工作原理

核心思想:**把要走代理的流量,在 DNS 这一层解析成网关自己的 IP**,让它自然 funnel 进网关上的 xray;不走代理的流量则返回真实 IP,客户端直连,网关完全不经手。
...
| 场景 | DNS 行为 | 数据路径 |
|---|---|---|
| 命中强制代理域名表 | `address` 直接返回网关 IP | 客户端 → xray → 直出 |
| 未命中,解析出国外 IP | `ip-alias` 改写成网关 IP | 客户端 → xray → 直出 |
...
- **直出**:被代理流量经 xray dokodemo-door 透明转发(不解密 TLS),直接走网关默认路由出网——无 mark / 隧道 / 多出口 / 策略路由。
