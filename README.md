...
- **Review before every plugin write**: the bot shows the complete normalized impact first, then issues a short-lived one-use confirmation bound to the administrator, exact private chat, operation, current revision, and exact extension snapshot or marketplace index digest. Expired, replayed, stale, cross-user, cross-chat, or digest-mismatched confirmations fail closed. Install and update apply always finish disabled; enable is a separate confirmation.
