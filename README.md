...
- **Native interception extensions and marketplace**: the dedicated Console page accepts only strict `5gpn.io/v1` YAML manifests from one HTTPS URL, a local paste/upload, or an explicitly selected entry from an operator-added `5gpn.io/marketplace/v1` index. Marketplace refreshes are bounded, SSRF-guarded, atomically cached, and never install or enable anything automatically. Selecting an entry refetches the manifest through the native parser and verifies the published manifest/script digests before storing the ordinary disabled immutable snapshot. A manifest explicitly declares immutable metadata, `captureHosts`, structured request/response actions, typed settings, optional bounded storage, exact HTTP(S) network origins, an optional operator-group requirement, and safe upstream mappings. Unknown fields and unsafe YAML features fail installation; there is no third-party client-format compatibility layer. Every script defines `transform(context)` and runs in a fresh bounded goja VM with no filesystem, process, timer, module-loader, socket, or ambient `fetch`. Only an explicitly declared and confirmed origin list exposes synchronous `context.network.request`; all such traffic still returns through mihomo. First-party source remains external in [`moooyo/5gpn-extensions`](https://github.com/moooyo/5gpn-extensions), whose official marketplace URL is `https://moooyo.github.io/5gpn-extensions/marketplace/v1/index.json`; this core repository does not vendor or release extension code.
