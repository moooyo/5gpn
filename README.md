...
- **控制面**:Phase 3 已实现——控制面现为 **:9443 HTTPS REST API + 内嵌 Web UI**(React,`go:embed`),bearer token 鉴权、仅 CLIENT_NET 可达;**Telegram bot 现为 5gpn-dns Go 守护进程内的组件**(`github.com/go-telegram/bot`,`TGBOT_TOKEN`/`TGBOT_ADMINS` 管理员门控),直接调进程内 `Controller`(不走 :9443 / 不需 token),与 Web UI 并存;install.sh 自动生成 `DNS_API_TOKEN`(空则禁用 HTTP 控制面,不会无鉴权对外提供)。**已无任何 Python**。
...
- **Telegram bot 进程内**:Telegram 控制 bot 现为 `5gpn-dns` 守护进程内的 Go goroutine(`github.com/go-telegram/bot`),直接调用进程内 `Controller`(不经 :9443 / 不需 token),与 Web UI 并存;特权操作(服务控制、证书续期)委托 systemd,不放松 5gpn-dns 沙箱。
...
sudo bash install.sh --setup-tgbot   # 启用进程内 Telegram 控制 bot(写 TGBOT_TOKEN/TGBOT_ADMINS 到 dns.env 并重启 5gpn-dns;Gum TUI 交互)
```
...
| `cmd/5gpn-dns/bot.go` | 进程内 Telegram 控制 bot(`github.com/go-telegram/bot`,管理员门控,直接调 `Controller`;与 Web UI 并存;特权操作委托 systemd) |
| `cmd/5gpn-dns/iosd.go` | iOS 描述文件分发的进程内 HTTP 服务(:8111,`WWW_DIR`/`DNS_IOS_LISTEN`) |
