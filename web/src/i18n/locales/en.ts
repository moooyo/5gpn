...
const en = {
  ...
  topbar: {
    ...
    sub: {
      ...
      pluginLogs: 'Live plugin-engine output · script console and action execution events · read-only',
  ...
  setupGuide: {
    ...
    interceptCA: {
      ...
      trustWarningTitle: 'Intercepted apps require certificate trust',
      trustWarningBody: 'Installing the DoT or CA profile is not enough. Open Settings → General → About → Certificate Trust Settings and enable full trust for the 5gpn root, or intercepted apps such as YouTube will fail TLS validation and may not load.',
      trustWarningAction: 'Download MITM root',
  ...
  settings: {
    ...
    statsTitle: 'Cumulative statistics',
    statsHint: 'Dashboard counters accumulate from this installation’s first boot and are restored on every restart. Nothing ages out, so the per-upstream average latency is an unweighted mean over every exchange ever recorded — one pathological query stays visible in it indefinitely.',
    statsQueries: 'Queries counted',
    statsQueriesHint: 'Total since the counters were last cleared',
    statsSamples: 'Upstream exchanges',
    statsSamplesHint: 'The population the average latencies are computed over',
    statsReset: 'Reset statistics',
    statsResetConfirmTitle: 'Reset all cumulative statistics?',
    statsResetConfirmBody: 'Zeroes every counter — query totals, decision breakdown, cache hit rate and both upstream latency averages — and writes the cleared set to disk immediately. Resolution is unaffected; only the numbers the dashboard reports are discarded. This cannot be undone.',
    statsResetDone: 'Statistics cleared.',
    ...
    upstreamsEndpoint: 'DoH endpoint URL',
    upstreamsEndpointHint: 'The resolver’s RFC 8484 endpoint, e.g. https://dns.google/dns-query. Its hostname is used for TLS verification; the address below is what gets dialled.',
    upstreamsEndpointRequired: 'Enter the DoH endpoint URL.',
    ...
    upstreamsAddressInvalid: 'Enter a valid IPv4 address with an optional port from 1 to 65535.',
    ...
    ingressReason: {
      'invalid-config': 'The mihomo config does not satisfy 5gpn’s seven infrastructure invariants, so no ingress module can be read or changed — this is about the file as a whole, not this module. Check the external controller and its secret, the gateway inbound, the loopback DNS broker, the console and zashboard SNI splits, and the anti-loop denies in',
      'alias-or-merge-conflict': 'The YAML uses anchors, aliases or merge keys (&name, *name, <<:) somewhere in the document. 5gpn edits rules positionally and will not rewrite an aliased file. Expand them into literal YAML in',
      'listener-structure-conflict': 'The listeners section is not the plain sequence 5gpn writes, so it will not be modified. Review it in',
      'sniffer-structure-conflict': 'The sniffer section is not the plain structure 5gpn writes, so it will not be modified. Review it in',
      'rules-structure-conflict': 'The rules section is missing, is not a sequence, or contains an entry that is not a plain one-line string. Every rule must be a scalar such as "- AND,((NETWORK,UDP),(DST-PORT,443)),REJECT". Fix it in',
      'terminal-match-missing': 'The rules list needs exactly one MATCH rule and it must be the final entry, with no extra parameters and a proxy group that exists. Fix it in',
      'canonical-gateway-missing': 'The gateway inbound 5gpn manages is absent from the config. Restore it in',
      'canonical-gateway-conflict': 'The gateway inbound has been edited away from the shape 5gpn manages, so it will not be overwritten. Review it in',
      'fail-closed-guards-missing': 'The fail-closed guard rules are missing, so enabling this module could open traffic it is meant to contain. Restore them in',
      'interception-egress-terminator-missing': 'The rule "IN-NAME,intercept-egress,REJECT" is missing. This module is positioned immediately after it, so without it 5gpn cannot place the rule. Restore it in',
      'interception-egress-terminator-duplicate': 'The rule "IN-NAME,intercept-egress,REJECT" appears more than once, so 5gpn cannot tell where this module belongs. Remove the duplicate in',
      'partial-or-custom-5060': 'The :5060 listener, its sniffers, or its rules are present but not in the exact shape 5gpn manages, so it will not be overwritten. Reconcile them in',
      'partial-or-custom-quic-block': 'A UDP/443 rule is present but is not the single canonical one directly after "IN-NAME,intercept-egress,REJECT". Any rule combining (NETWORK,UDP) with (DST-PORT,443) counts — including your own per-site QUIC rules — and a second one locks this toggle even when blocking still works. Reconcile them in',
    },
  ...
  extensions: {
    ...
    statusNotEffective: 'This extension is not in effect — the gateway refused to publish interception routing',
    statusReason: {
      'mihomo-management-unavailable': 'mihomo management is unavailable, so no interception routing can be published',
      'mihomo-config-unreadable': 'The mihomo configuration could not be read, so interception routing was withdrawn',
      'proxy-groups-structure-conflict': 'The mihomo proxy-groups block is not in a shape this gateway can read',
      'credential-mismatch': 'The mihomo listener credentials no longer match the sidecar document — reinstall to realign them',
      'certificate-not-ready': 'The interception certificate for these capture hosts has not been published yet',
      'overlay-client-anchor-missing': 'The mihomo config has no RUNTIME-OVERLAY,5gpn,client anchor — run 5gpn mihomo-reset',
      'overlay-client-anchor-duplicate': 'The mihomo config carries more than one client overlay anchor',
      'overlay-client-anchor-after-match': 'The client overlay anchor sits below the terminal MATCH, so captured traffic never reaches it',
      'overlay-client-anchor-before-terminator': 'The client overlay anchor sits above the fail-closed egress terminator, inverting rule precedence',
      'overlay-egress-anchor-missing': 'The mihomo config has no RUNTIME-OVERLAY,5gpn,egress anchor — run 5gpn mihomo-reset',
      'overlay-egress-anchor-duplicate': 'The mihomo config carries more than one egress overlay anchor',
      'overlay-egress-anchor-misplaced': 'A rule sits between the egress anchor and the fail-closed terminator',
      'interception-egress-terminator-missing': 'The fail-closed IN-NAME,intercept-egress,REJECT rule is missing',
      'interception-egress-terminator-duplicate': 'The mihomo config carries more than one fail-closed egress terminator',
    },
    settingsConflict: 'Another change was published while this dialog was open, so this save was refused. Your entries are kept — press Save again to apply them to the current state.',
    ...
    networkNone: 'No network access requested.',
    ...
    networkChip: 'Any host',
    networkWarning: 'This extension holds the network grant, which names no addresses: the permission cannot state where it may reach. It can send any decrypted request, response, setting, or stored data it can read to any host it resolves, and a rewritten request forwards the complete method, decoded body, and end-to-end headers, which may include Cookie and Authorization credentials. This cannot be narrowed after enabling.',
  ...
  marketplace: {
    ...
    sourceUnreadable: 'Its cached index cannot be read',
    brokenTitle_one: '{{count}} marketplace source cannot be read',
    brokenTitle_other: '{{count}} marketplace sources cannot be read',
    brokenBody: 'Their cached index was written against an older contract, so nothing can be installed from them until they are refreshed. Refresh or remove one from the menu on its chip below. Every other source is unaffected.',
  ...
  pluginLogs: {
    ...
    disconnectedBanner: 'Live logs disconnected · reconnecting every 3 seconds; new entries will resume after recovery',
    inactive: 'The interception sidecar is not currently expected to run',
    live: 'Live',
    paused: 'Paused',
    ...
    pluginFilterLabel: 'Filter by plugin',
    searchLabel: 'Search plugin logs',
    searchPlaceholder: 'Search message, plugin, or action…',
    engineTag: 'Engine',
    colTime: 'Time',
    colLevel: 'Level',
    colPluginAction: 'Plugin · action',
    colMessage: 'Message',
    detail: {
      pluginId: 'Plugin ID',
      phase: 'Phase',
    ...
    emptyTitle: 'No plugin logs yet',
    emptyHint: 'Waiting for plugin-engine output, or adjust the current filters…',
    emptyAction: 'No action',
    count: '{{count}} entries',
    footer: '1,000-entry ring buffer · memory only, never written to disk',
    footerMobile: '{{count}} entries · 1,000-entry ring buffer · memory only',
    transport: 'wss ticket · /intercept/logs',
    durationMs: '{{value}}ms',
    expandRow: 'Expand log details',
    collapseRow: 'Collapse log details',
    level: {
      info: 'info',
      warn: 'warn',
      error: 'error',
    ...
    missingValue: '—',
    goToExtensions: 'Open Extensions',
    pluginsLoadFailed: 'Could not load the installed plugin list.',
  },
