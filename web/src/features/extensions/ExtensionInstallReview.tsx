...
export function ExtensionInstallReview({ module }: { module: InterceptModule }) {
  ...
  return (
    <div className="space-y-3" data-testid="extension-install-review">
      ...
      <Row label={t('extensions.networkOriginsTitle')}>
        {module.network_origins.length ? <div className="flex flex-wrap gap-1.5">{module.network_origins.map((origin) => <code key={origin} className="max-w-full break-all rounded-chip bg-surface-container-low px-2 py-1 font-mono text-meta text-text-mid">{origin}</code>)}</div> : <p className="text-meta text-text-faint">{t('extensions.networkOriginsNone')}</p>}
      </Row>
