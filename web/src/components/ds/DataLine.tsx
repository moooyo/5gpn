  ...
  return (
    <div className={cn('flex items-center justify-between gap-4 border-b border-divider py-3.5 last:border-b-0', className)}>
      <div className="flex flex-col gap-0.5">
        ...
        {sub !== undefined ? <span className="text-label leading-5 text-text-faint">{sub}</span> : null}
      </div>
...
export function SectionLabel({ children, className }: SectionLabelProps) {
  return (
    <div className={cn('text-label font-medium tracking-[.06em] text-text-faint', className)}>{children}</div>
  )
}
