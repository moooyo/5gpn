...
describe('api client — live methods', () => {
  ...
  it('uses bearer-protected kernel health, one-use log tickets, and zashboard handoff endpoints', async () => {
    vi.stubEnv('VITE_API_MOCK', '0')
    vi.resetModules()
    const f = vi.fn()
      .mockResolvedValueOnce(jsonResp(200, { version: 'v1.19.28', meta: true }))
      .mockResolvedValueOnce(jsonResp(200, { running: true, expected: true, installed_plugins: 3, active_plugins: 2 }))
      .mockResolvedValueOnce(jsonResp(200, { ticket: 'mihomo-once' }))
      .mockResolvedValueOnce(jsonResp(200, { ticket: 'plugin-once', expires_in_seconds: 30 }))
      .mockResolvedValueOnce(jsonResp(200, { url: 'https://zash.example/handoff?ticket=once', expires_in_seconds: 30 }))
    vi.stubGlobal('fetch', f)
    const { api } = await import('./client')
    const pluginTicketController = new AbortController()

    await expect(api.getMihomoHealth()).resolves.toMatchObject({ version: 'v1.19.28' })
    await expect(api.getInterceptHealth()).resolves.toMatchObject({ running: true, active_plugins: 2 })
    await expect(api.createMihomoLogTicket()).resolves.toEqual({ ticket: 'mihomo-once' })
    await expect(api.createPluginLogTicket(pluginTicketController.signal)).resolves.toEqual({ ticket: 'plugin-once', expires_in_seconds: 30 })
    ...
    expect(f.mock.calls[1][0]).toBe('/api/intercept/health')
    expect(f.mock.calls[2][0]).toBe('/api/mihomo/log-ticket')
    expect(f.mock.calls[2][1].method).toBe('POST')
    expect(f.mock.calls[3][0]).toBe('/api/intercept/logs/ticket')
    expect(f.mock.calls[3][1].method).toBe('POST')
    expect(f.mock.calls[3][1].signal).toBe(pluginTicketController.signal)
    expect(f.mock.calls[4][0]).toBe('/api/mihomo/zashboard-handoff')
    expect(f.mock.calls[4][1].method).toBe('POST')
  })
})
