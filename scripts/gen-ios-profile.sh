...
# Both payloads are completed in one private staging directory. Container mode
# keeps the signing-key snapshot in tmpfs; only verified public artifacts cross
# into the unpublished persistent generation.
if [[ "${FIVEGPN_RUNTIME:-}" == container \
   && "${EUID:-$(id -u)}" == 10001 \
   && "$(id -g)" == 10001 ]]; then
    profile_stage_parent=/run/5gpn
    [[ -d "$profile_stage_parent" && ! -L "$profile_stage_parent" \
       && "$(stat -c '%u:%g:%a' -- "$profile_stage_parent" 2>/dev/null)" == 10001:10001:700 ]] \
        || { err "The private container profile staging root is unsafe."; exit 1; }
else
    profile_stage_parent="$OUTPUT_DIR"
fi
stage_dir="$(mktemp -d "${profile_stage_parent}/.ios-profile.XXXXXX")" \
    || { err "Could not create the iOS profile staging directory."; exit 1; }
chmod 0700 "$stage_dir" \
    || { rmdir -- "$stage_dir" 2>/dev/null || true; exit 1; }
profile_stage_marker="${stage_dir}/.5gpn-profile-stage"
printf '%s\n' '5gpn-profile-stage-v1' > "$profile_stage_marker" \
    && chmod 0600 "$profile_stage_marker" \
    || { rm -f -- "$profile_stage_marker"; rmdir -- "$stage_dir" 2>/dev/null || true; exit 1; }
...
profile_publish_candidate="${OUTPUT_DIR}/.ios-dot.mobileconfig.new.$$.$RANDOM"
intercept_publish_candidate="${OUTPUT_DIR}/.ios-intercept-ca.mobileconfig.new.$$.$RANDOM"
inputs_publish_candidate="${OUTPUT_DIR}/.5gpn-profile-inputs.new.$$.$RANDOM"
...
cleanup_profile_stage() {
    ...
    rm -f -- \
        ...
        "$profile_stage_marker" \
        "$profile_publish_candidate" "$intercept_publish_candidate" \
        "$inputs_publish_candidate" \
        2>/dev/null || rc=1
...
profile_expected_root_uid() {
    if [[ "$DOT_CERT_ROLE" == /etc/5gpn/cert/dot \
       && "${FIVEGPN_RUNTIME:-}" == container \
       && "${EUID:-$(id -u)}" == 10001 \
       && "$(id -g)" == 10001 ]]; then
        printf '10001\n'
    elif [[ "$DOT_CERT_ROLE" == /etc/5gpn/cert/dot ]]; then
        printf '0\n'
    else
        printf '%s\n' "${EUID:-$(id -u)}"
    fi
...
if ! install -m 0644 -- "$staged_profile" "$profile_publish_candidate" \
   || ! mv -Tf -- "$profile_publish_candidate" "$profile_path"; then
    err "Could not place the signed iOS profile in the unpublished generation."
    exit 1
fi
if ! install -m 0644 -- "$staged_intercept_profile" "$intercept_publish_candidate" \
   || ! mv -Tf -- "$intercept_publish_candidate" "$intercept_profile_path"; then
    err "Could not place both signed profiles in the unpublished generation."
    exit 1
fi
if ! install -m 0644 -- "$staged_profile_inputs" "$inputs_publish_candidate" \
   || ! mv -Tf -- "$inputs_publish_candidate" "$profile_inputs_path"; then
