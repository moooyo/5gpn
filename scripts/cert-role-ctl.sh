...
cert_role_ctl_expected_uid() {
    if [[ "$CERT_ROLE_CTL_ROOT" == "$CERT_ROLE_CTL_PRODUCTION_ROOT" \
       && "$CERT_ROLE_CTL_CONTAINER_MODE" == 1 \
       && "${FIVEGPN_RUNTIME:-}" == container \
       && "${EUID:-$(id -u)}" == 10001 \
       && "$(id -g)" == 10001 ]]; then
        printf '10001\n'
    elif [[ "$CERT_ROLE_CTL_ROOT" == "$CERT_ROLE_CTL_PRODUCTION_ROOT" ]]; then
        printf '0\n'
    else
        [[ "$CERT_ROLE_CTL_ALLOW_TEST_ROOT" == 1 ]] || return 1
        printf '%s\n' "${EUID:-$(id -u)}"
...
cert_role_ctl_config_root_is_safe() {
    ...
    if [[ "$CERT_ROLE_CTL_ROOT" == "$CERT_ROLE_CTL_PRODUCTION_ROOT" ]]; then
        if [[ "$CERT_ROLE_CTL_CONTAINER_MODE" == 1 \
           && "${FIVEGPN_RUNTIME:-}" == container \
           && "$expected_uid" == 10001 && "$expected_gid" == 10001 ]]; then
            [[ "$mode" == 750 && "$actual_gid" == 10001 ]] || return 1
        elif [[ "$mode" == 755 ]]; then
...
cert_role_ctl_remove_role_candidate_exact() { # <candidate> <role>
    ...
    publication_fs_plain_file_is_safe "$marker" "$expected_uid" "$expected_root_gid" 644 \
        && printf '%s\n' "$CERT_ROLE_CTL_ROLE_VALUE_PREFIX:$role" | cmp -s - "$marker" || return 1
    for entry in "${entries[@]}"; do
        name="$(basename -- "$entry")"
        case "$name" in
...
cert_role_ctl_role_is_recoverable() { # <role>
    ...
    publication_fs_plain_file_is_safe "$role_root/$CERT_ROLE_CTL_ROLE_MARKER" \
        ...
        && printf '%s\n' "$CERT_ROLE_CTL_ROLE_VALUE_PREFIX:$role" \
            | cmp -s - "$role_root/$CERT_ROLE_CTL_ROLE_MARKER" \
        || return 1
...
cert_role_ctl_role_boundary_is_safe() {
    ...
    publication_fs_plain_file_is_safe "$marker" "$expected_uid" "$expected_root_gid" 644 \
        && printf '%s\n' "$CERT_ROLE_CTL_ROLE_VALUE_PREFIX:$role" | cmp -s - "$marker" \
        || return 1
