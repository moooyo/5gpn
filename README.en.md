...
`auto` queries the China and trust upstream groups concurrently. Members within each group are attempted sequentially in configured order with fair slices of the remaining deadline. Fresh installations have the installer seed `223.5.5.5:53` and `22.22.22.22:53` into `upstreams.json` — the sole source of truth for the upstream groups, with no `dns.env` counterpart — after which they can be changed in the Console and hot-apply without a restart. A member of either group declares its transport by spec form: a bare `IP[:port]` is plain UDP, `serverName@IP[:port]` is DoT, and `https://host/path@IP[:port]` is DoH over a pooled HTTP/2 connection, which reuses its connection so a query costs one round trip instead of three. Transport is a per-member property, so one group may mix all three. Plain UDP carries no query-name anti-spoof mechanism, so prefer DoT or DoH wherever the resolver offers it. A queries follow the policy above; AAAA, HTTPS, and SVCB return NODATA with authority data. Other types go to the trust group with AAAA, HTTPS, and SVCB records removed from every section before the reply is cached or returned - refusing those RRsets only when they are the question would leave them reachable under another qtype, since ANY returns both outright and MX/NS/SRV/PTR replies carry the target AAAA as additional-section glue.
