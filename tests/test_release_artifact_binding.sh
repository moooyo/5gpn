...
if grep -Fq 'README.en.md' "$RELEASE" \
   ...
   && grep -Fq 'docs/docker.md' "$RELEASE" \
   && grep -Fq 'docs/native-extensions.md' "$RELEASE" \
   && grep -Fq 'tests/container-acceptance.sh' "$RELEASE" \
   ...
   && grep -Fq 'tests/acceptance/disruption-recovery.md' "$RELEASE"; then
    pass "installer bundle retains centralized pins, Docker acceptance, both README languages, and linked operator runbooks"
else
