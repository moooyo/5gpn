...
printf '%s' "$dbg" | grep -Fq 'remove_owned_unit 5gpn-certbot-renew.timer' \
    || fail "install.sh: debug branch does not ownership-gate renewal timer removal"
printf '%s' "$dbg" | grep -Fq 'remove_owned_unit 5gpn-certbot-renew.service' \
    || fail "install.sh: debug branch does not ownership-gate renewal service removal"
